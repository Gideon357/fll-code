...
class Menu:
    ...
    def __init__(self, start_page=0, menu_pages=[], debug_on=True):
        ...
        self.current_page = (
            start_page  # Represents the current page/list of menu items being displayed
        )
        self.menu_pages = (
            menu_pages  # A list of lists for all of the pages for the menu items
        )
    ...
    def menu_tone(self):
        player = Sound()
        player.play_tone(
            1000, 0.5, delay=0.0, volume=100, play_type=Sound.PLAY_NO_WAIT_FOR_COMPLETE
        )
    ...
    def press_tone(self):
        ...
        player.play_tone(
            1250, 0.2, delay=0.0, volume=100, play_type=Sound.PLAY_NO_WAIT_FOR_COMPLETE
        )
            ...
            if pressed in menu_page:
                if last == pressed:  # was same button pressed?
                    ...
                    try:
                        ...
                        self.debug(
                            "Type of run_function: {}".format(type_of_run_function)
                        )
